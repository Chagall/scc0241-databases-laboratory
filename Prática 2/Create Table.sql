...
CREATE TABLE Prefeito (
  ...
  CONSTRAINT CHECK_VICE_PREFEITO CHECK (UPPER(vice) in ('T', 'F'))
);
...
CREATE TABLE Governador (
  ...
  CONSTRAINT FK_GOVERNADOR_ESTADUAL FOREIGN KEY (cargo) REFERENCES Estadual(cargo) ON DELETE CASCADE,
    CONSTRAINT CHECK_VICE_GOVERNADOR CHECK (UPPER(vice) in ('T', 'F'))
);
...
CREATE TABLE Presidente (
  ...
  CONSTRAINT CHECK_VICE_PRESIDENTE CHECK (UPPER(vice) in ('T', 'F'))
);
